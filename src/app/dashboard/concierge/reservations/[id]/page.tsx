...
export default async function ReservationDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const account = await requireAccount(`/dashboard/concierge/reservations/${id}`);
  ...
  const hasCharge = !!r.depositRequirement; // a real charge only when the concierge recorded terms

  return (
    <>
      <div className="pg-head">
        <Link href="/dashboard/concierge/reservations" className="cx-back">← My Reservations</Link>
      ...
      {/* Full intake payload, for transparency. */}
      ...
      {/* Purchase & approval safety — shown ONLY when a real charge/terms have
          been recorded by the concierge. We never charge from a chat/voice
          request; the exact terms are shown and confirmation is explicit. */}
      {hasCharge && (
        <section className="sec cx-pay">
          <div className="sec__h"><h2 className="sec__t">Payment &amp; approval</h2></div>
          <p className="cx-pay__intro">This reservation requires a payment. Please review the exact terms before confirming — you will never be charged from a chat or voice request.</p>
          <div className="cx-detail">
        ...
        <div className="cx-actionbar">
          ...
          {clientCanCancel(r.status) && (
            <form action={cancelReservationAction}>
              <input type="hidden" name="id" value={r.id} />
              <button type="submit" className="btn btn--ghost cx-danger">Cancel Request</button>
            </form>
          )}
        </div>
        {r.status === "CONFIRMED" && (
