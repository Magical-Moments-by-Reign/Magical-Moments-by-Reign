...
import { readProfileDiagnostic } from "@/lib/spotify/diagnostics";
...
export default async function SpotifyDiagnosticPage() {
  ...
  return (
    <div style={{ padding: "2rem", maxWidth: 720, fontFamily: "monospace", lineHeight: 1.7 }}>
      ...
      <hr />
      <h2>5b. Latest safe /v1/me diagnostic</h2>
      ...
      <p>can use this app — this is a per-Spotify-account allow-list, separate from your Spotify Developer account itself (being the app&rsquo;s developer does not automatically allow-list your personal listening account). Spotify changed the allow-list size in their February–March 2026 Development Mode migration — check the actual number shown on the User Management page itself rather than trusting a remembered figure; it has been reported as low as 5 (down from the previous 25) for apps on the new quota rules. Add the exact email address tied to whichever Spotify account will be used to test the Connect flow.</p>
      <p><strong>Important correction:</strong> a non-allow-listed account can still complete Spotify&rsquo;s consent screen and this app can still exchange the code for a valid access token — Development Mode does not block that step. What it blocks is the Web API calls made <em>with</em> that token: a request like <code>GET /v1/me</code> (the profile fetch right after connecting) returns HTTP 403 for a non-allow-listed account, which surfaces here as <code>?spotify=profile_failed</code>. If you see that status, the fix is adding the account to User Management, not something in this app&rsquo;s code.</p>
