...
import { spotifyClientId, spotifyClientSecret, spotifyRedirectUri, SPOTIFY_SCOPES } from "@/lib/spotify/config";
import { getAppAccessToken } from "@/lib/spotify/oauth";
import { searchCatalog } from "@/lib/spotify/catalog";
import { readProfileDiagnostic, readTokenDiagnostic } from "@/lib/spotify/diagnostics";
...
export default async function SpotifyDiagnosticPage() {
  ...
  // ── 5b. Latest real GET /v1/me diagnostic from an actual member's Connect
  //      Spotify attempt — persisted by fetchSpotifyProfile() in oauth.ts.
  //      Never populated by this page itself; this only reads what a live
  //      human click-through already recorded. */
  const profileDiag = await readProfileDiagnostic();

  // ── 5c. Latest safe token-exchange diagnostic — recorded by
  //      exchangeCodeForTokens() in oauth.ts right after a real code
  //      exchange, so the actual scope Spotify granted (not just what was
  //      requested) is visible without ever showing the token itself. */
  const tokenDiag = await readTokenDiagnostic();
  const requestedScopes = SPOTIFY_SCOPES.join(" ");
  const returnedScopeSet = new Set((tokenDiag?.scopeReturned ?? "").split(" ").filter(Boolean));
  const missingScopes = SPOTIFY_SCOPES.filter((s) => !returnedScopeSet.has(s));
  ...
  return (
    <div style={{ padding: "2rem", maxWidth: 720, fontFamily: "monospace", lineHeight: 1.7 }}>
      ...
      <hr />
      <h2>5b. Latest safe token-exchange diagnostic (Authorization Code user token)</h2>
      <p>Recorded by <code>exchangeCodeForTokens()</code> right after a real code exchange — this is the USER token, never the Client Credentials app token from step 2 above (different function, different code path — see the source-trace note below). Never contains the token itself.</p>
      <Row label="Requested scopes (SPOTIFY_SCOPES)" value={requestedScopes} />
      {!tokenDiag ? (
        <Row label="Status" value="No token exchange recorded yet — click Connect Spotify at least once" warn />
      ) : (
        <>
          <Row label="User access token present" value={tokenDiag.accessTokenPresent ? "YES" : "NO"} warn={!tokenDiag.accessTokenPresent} />
          <Row label="token_type" value={tokenDiag.tokenType ?? "(none)"} />
          <Row label="expires_in" value={tokenDiag.expiresIn != null ? String(tokenDiag.expiresIn) : "(none)"} />
          <Row label="Scope returned by Spotify" value={tokenDiag.scopeReturned || "(empty)"} warn={!tokenDiag.scopeReturned} />
          <Row label="Scopes requested but NOT returned" value={missingScopes.length ? missingScopes.join(" ") : "none — all requested scopes were granted"} warn={missingScopes.length > 0} />
          <Row label="Recorded at" value={tokenDiag.recordedAt} />
        </>
      )}

      <hr />
      <h2>5c. Latest safe /v1/me diagnostic</h2>
      <p>Recorded automatically by the real production callback the last time a member (or you) clicked Connect Spotify and reached the profile-fetch step — not generated by loading this page. Never contains a token, code, or personal profile data.</p>
      {!profileDiag ? (
        <Row label="Status" value="No attempt recorded yet — click Connect Spotify on /dashboard/discovery/music at least once" warn />
      ) : (
        <>
          <Row label="Profile request attempted" value={profileDiag.requestAttempted ? "YES" : "NO"} />
          <Row label="Profile HTTP status" value={profileDiag.httpStatus != null ? String(profileDiag.httpStatus) : "none (network failure)"} warn={profileDiag.category !== "none"} />
          <Row label="Response content type" value={profileDiag.contentType ?? "(none)"} />
          <Row label="Response body present" value={profileDiag.bodyPresent ? "YES" : "NO — Spotify returned an empty body"} warn={!profileDiag.bodyPresent} />
          <Row label="Response body length" value={`${profileDiag.bodyLength} bytes`} />
          <Row label="JSON parse" value={profileDiag.jsonParsed ? "PASS" : "FAIL"} warn={!profileDiag.jsonParsed} />
          {profileDiag.rawBodyPreview && <Row label="Raw body preview (first 200 chars, non-JSON only)" value={profileDiag.rawBodyPreview} warn />}
          <Row label="Spotify safe error code" value={profileDiag.spotifyErrorCode ?? "(none)"} warn={!!profileDiag.spotifyErrorCode} />
          <Row label="Spotify safe error message" value={profileDiag.spotifyErrorMessage ?? "(none)"} warn={!!profileDiag.spotifyErrorMessage} />
          <Row label="Error category" value={profileDiag.category} warn={profileDiag.category !== "none"} />
          <Row label="Response contains Spotify id" value={profileDiag.containsId ? "YES" : "NO"} warn={!profileDiag.containsId} />
          <Row label="Recorded at" value={profileDiag.recordedAt} />
        </>
      )}
      ...
      <p>can use this app — this is a per-Spotify-account allow-list, separate from your Spotify Developer account itself (being the app&rsquo;s developer does not automatically allow-list your personal listening account). Spotify changed the allow-list size in their February–March 2026 Development Mode migration — check the actual number shown on the User Management page itself rather than trusting a remembered figure; it has been reported as low as 5 (down from the previous 25) for apps on the new quota rules.</p>
      <p><strong>If Premium and User Management are both already confirmed and /v1/me still returns 403:</strong> that specific combination — an approved, Premium, freshly-relogged-in test account still getting 403 on <code>GET /v1/me</code> — has been reported by other developers as a live, documented issue following Spotify&rsquo;s February–March 2026 Development Mode migration, not something this app&rsquo;s code can work around. Use sections 5b/5c above to confirm the request itself is correct (the real user token, the requested scopes, an empty or non-JSON 403 body) before assuming anything on this side is wrong — if everything there checks out, this is a Spotify-side platform issue to raise with Spotify support/community, not a code bug here.</p>
