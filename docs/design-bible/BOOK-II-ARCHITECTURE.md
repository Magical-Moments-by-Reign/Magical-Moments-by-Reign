...
| **Family Connections** (one membership, whole family) | **Built & tested (8 unit tests)** — host-controlled guest permissions, one-membership rule (guests participate free), graceful premium-preview locks (Magical Preview Pass), video-gathering/timeline/feed catalogs, opt-in **general-location** Family Map (liveLocationAllowed=false), per-moment privacy (canAccessMoment) · schema: FamilyGuest, FamilyTimelineEntry, FamilyFeedPost, FamilyGathering, FamilyMapLocation · auth, storage, video providers & UI gated · never live tracking — see [standard](./STANDARD-family-connections.md) |
| **Family Birthday & Celebration Network** | **Built & tested (12 unit tests)** — auto calendar (visible-only), monthly view, yearly + leap-year recurrence (nextOccurrence/resolveLeapDay), reminder schedule (14/7/2/morning +optional 30), group prefs, manual non-member entries (linkable later), one-touch birthday actions, Ask Magical digest · schema: CelebrationEntry + CelebrationReminderPref · notifications/calendar-export/one-touch integrations gated · privacy: only shared celebrations appear — see [standard](./STANDARD-celebration-network.md) |
...
| **Platform Foundation** (accounts · auth · permissions · notifications) | **Service layer built & tested (17 unit tests)** — roles.ts (canonical PlatformRole; delegates to existing permission sets; child safeguards), auth.ts (scrypt hash + hashed sessions + recover-before-duplicate signup + guardian rule), notifications.ts (provider-agnostic dispatcher; in-app always, email/SMS/push seams; per-type/per-channel prefs; minors in-app only), invitations.ts (hashed-token invites; accept decisions) · schema: Account +platformRole/+guardianAccountId, Session, Notification, NotificationPreference, Invitation · login/OAuth UI + cookie/middleware, email/SMS/push providers, and a reminder scheduler are gated seams — see [standard](./STANDARD-platform-foundation.md) |
